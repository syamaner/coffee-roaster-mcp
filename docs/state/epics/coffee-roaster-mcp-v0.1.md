...
- PR review hardening for E3-S9:
  ...
  - Follow-up Codex review found that hidden safe cleanup violated the emergency-stop opt-in contract, that control-step readiness needed fresh command-write progress rather than stale write counters, and that heat/fan steps also needed to verify the resulting driver state matched the requested target.
  - Hardened the validation harness to validate heat/fan/durations and output path before connecting, require finite durations, capture one stable-telemetry snapshot, abort before controls when telemetry does not pass, write partial failure reports before re-raising, record disconnect failures, drop before cooling-stop validation in full runs, honor the skipped emergency-stop contract without hidden emergency-stop commands, require per-step command-write progress plus requested heat/fan target state, and base readiness on required passed steps plus no failed steps.
