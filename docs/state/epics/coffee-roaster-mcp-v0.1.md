...
- Validation run for E3-S4:
  ...
  - Review hardening corrected Hottop capability flags for not-yet-implemented controls, threaded configured port/baudrate/command interval through `build_server_context`, removed the host-specific default port, guaranteed serial close on join timeout, and blocked reconnect until a previous command loop is fully stopped.
  - Ran `./.venv/bin/python -m pytest tests/test_drivers.py tests/test_package.py`: 56 passed.
  - Ran `./.venv/bin/python -m pytest`: 106 passed.
  - Ran `./.venv/bin/python -m ruff check .`: passed.
  - Ran `./.venv/bin/python -m ruff format --check .`: passed.
  - Ran `./.venv/bin/python -m pyright`: 0 errors.
