...
- `E3-S4` adds the first Hottop driver lifecycle slice. The driver can be constructed for `hottop_kn8828b_2k_plus`, opens serial transport lazily through pyserial, starts a command-loop lifecycle thread on connect, and disconnects by signalling the loop, joining it, closing serial transport, and clearing runtime references. Packet sending and hardware commands remain later Epic 3 stories.
...
- Validation run for E3-S4:
  ...
  - Ran `./.venv/bin/python -m pytest tests/test_drivers.py`: 39 passed.
  - Ran `./.venv/bin/python -m pytest`: 102 passed.
  - Ran `./.venv/bin/python -m ruff check .`: passed.
  - Ran `./.venv/bin/python -m ruff format --check .`: passed.
  - Ran `./.venv/bin/python -m pyright`: 0 errors.
