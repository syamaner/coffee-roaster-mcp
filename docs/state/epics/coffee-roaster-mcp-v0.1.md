...
- `E2-S5` now enforces phase-ordered roast events inside `RoastSessionStore`. New events must follow `pre_roast -> roasting -> development -> dropped -> cooling -> complete`, while repeated singleton calls keep their original event rows and fault rows remain appendable without resetting the first-fault timestamp.
